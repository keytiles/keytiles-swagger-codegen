...
## Bugfixes
 * We found a problem which comes to light if you have  
   1) an included schema from other file which you generate into a standalone package - which you add to importMappings  
   2) and in this package you use "oneOf" as a field in an object  
   This is turned into an "interface" (and not class) and is not excluded from the generated sources in the target generation
   Now this is solved by relaxing exclusion - see comment in ConfigOptionHelper.preprocessOpenAPIHook() method
